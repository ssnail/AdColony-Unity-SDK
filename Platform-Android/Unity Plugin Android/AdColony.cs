...
public class AdColony : MonoBehaviour 
{
  ...
  // DELEGATE PROPERTIES
  static public VideoStartedDelegate  OnVideoStarted;  // only called for iOS
  static public VideoFinishedDelegate OnVideoFinished;
  static public V4VCResultDelegate    OnV4VCResult;
  ...
  static public void Configure( string app_version, string app_id, params string[] zone_ids )
  {
    ...
    if (main_camera != null)
    {
      main_camera.AddComponent<AdColony>();
      configured = true;
    }
    else
    {
      Debug.LogError( 
        "AdColony requires a GameObject named \"Main Camera\" to attach to." );
    }
